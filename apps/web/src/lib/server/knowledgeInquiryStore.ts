...
function useDatabase(familyId: string) {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(familyId) && Boolean(createServiceSupabaseClient());
}
...
async function readDatabaseInquiries(familyId: string): Promise<KnowledgeInquiry[] | null> {
  ...
  return (data || []).map((row) => normalizeInquiry((row as { payload?: Partial<KnowledgeInquiry> }).payload || {}));
}
