...
LITE_ENV="$RUNTIME_ROOT/lite.env"
...
set_env "$LITE_ENV" FAMILY_APP_HOST "$LAN_HOST"
set_env "$LITE_ENV" FAMILY_APP_PORT "$APP_PORT"
set_env "$LITE_ENV" NEXT_PUBLIC_APP_URL "http://${LAN_HOST}:${APP_PORT}"

run_compose() {
  (cd "$PROJECT_ROOT" && docker compose --env-file "$LITE_ENV" -f docker-compose.lite.yml "$@")
}
