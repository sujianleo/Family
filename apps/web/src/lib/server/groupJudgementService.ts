...
export async function listGroupJudgements(context: Context, roomRecordId: string) {
  ...
  const client = createServiceExternalStoreClient() as any;
  ...
  await assertExternalStoreRoomMember(client, context, roomRecordId);
  await closeExpiredExternalStoreJudgements(client, context.familyId, roomRecordId);
  ...
  return Promise.all((data || []).map(async (row: any) => sanitizeGroupJudgementForViewer(await readExternalStoreJudgement(client, row), context.memberId)));
}
...
export async function getGroupJudgement(context: Context, id: string) {
  ...
  if (error) throw error;
  if (!data) return null;
  await assertExternalStoreRoomMember(client, context, data.room_record_id);
  return sanitizeGroupJudgementForViewer(await readExternalStoreJudgement(client, data), context.memberId);
}
...
export async function createGroupJudgement(context: Context, input: CreateInput) {
  ...
  if (!client || !isUuid(context.familyId)) {
    ...
    const room = await assertExternalStoreRoomMember(client, context, normalized.roomRecordId);
    const eligibleIds = await getExternalStoreEligibleMemberIds(client, context.familyId, room);
    assertSideMembers(eligibleIds, normalized.leftMemberId, normalized.rightMemberId);
    ...
    judgement = await readExternalStoreJudgement(client, data);
  }
...
export async function draftGroupJudgement(context: Context, input: { roomRecordId: string; statement: string }): Promise<GroupJudgementDraft> {
  ...
  if (!client || !isUuid(context.familyId)) {
    ...
    await assertExternalStoreRoomMember(client, context, input.roomRecordId);
  }
...
export async function closeGroupJudgement(context: Context, id: string) {
  ...
  if (!client || !isUuid(context.familyId)) {
    ...
    if (error) throw error;
    next = await readExternalStoreJudgement(client, data);
  }
...
async function assertExternalStoreRoomMember(client: any, context: Context, roomRecordId: string) {
...
async function getExternalStoreEligibleMemberIds(client: any, familyId: string, room: any) {
...
async function readExternalStoreJudgement(client: any, row: any): Promise<GroupJudgement> {
...
async function closeExpiredExternalStoreJudgements(client: any, familyId: string, roomRecordId: string) {
  ...
  for (const row of data || []) {
    const judgement = await readExternalStoreJudgement(client, row);
    const closedAt = new Date().toISOString();
