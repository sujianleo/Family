...
async function discoverOrganizationTargets(): Promise<OrganizationTarget[]> {
  ...
  if (!supabase) {
    const familyId = process.env.SUPABASE_DEFAULT_FAMILY_ID || "local-family";
    const memberId = process.env.SUPABASE_DEFAULT_MEMBER_ID || "me";
...
async function readLatestActivityAt(familyId: string, dataDir: string) {
  ...
  if (supabase && isUuid(familyId)) {
    const [events, records, messages] = await Promise.all([
      supabase.from("raw_events").select("created_at").eq("family_id", familyId).order("created_at", { ascending: false }).limit(1),
      supabase.from("family_records").select("updated_at").eq("family_id", familyId).order("updated_at", { ascending: false }).limit(1),
      supabase.from("room_messages").select("created_at").eq("family_id", familyId).order("created_at", { ascending: false }).limit(1)
    ]);
