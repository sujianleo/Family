...
import { assistantGrowthPrompt, evolveAssistantGrowth, type AssistantGrowthProfile } from "./assistantGrowth";
...
import { hasDeepSeekConfiguration, invokeDeepSeekJson, invokeDeepSeekText, searchDuckDuckGo, type DuckDuckGoSearchResult } from "./langchainAi";
...
import type { FamilyMember, FamilyRecord, TaskActionType } from "../types";
...
import { saveLiteFamilyRecord } from "./liteRepository";
...
async function runAutomationActionInternal(actionId: string, options: AutomationRunnerOptions = {}): Promise<AutomationRunResult> {
  ...
  if (action.id === "resource.assign_owner") {
    const familyId = options.familyId || "local-family";
    const recordId = readString(options.parameters?.record_id);
    const requestedMemberId = readString(options.parameters?.owner_member_id);
    const requestedName = readString(options.parameters?.owner_name);
    const members = await readFamilyMembersWithOverrides(options.dataDir || defaultDataDir);
    const owner = members.find((member) => member.id === requestedMemberId)
      || members.find((member) => member.displayName === requestedName);
    if (!recordId) throw new Error("缺少要修改归属的资料 ID。");
    if (!owner || owner.relationshipRole === "guest" || owner.householdRoles?.includes("assistant")) {
      throw new Error("没有找到可作为资料归属人的家庭成员。");
    }
    const store = createFamilyRecordStore();
    const existing = (await store.list(familyId)).find((record) => record.id === recordId);
    if (!existing) throw new Error("没有找到要修改归属的资料。");
    const executedAt = new Date().toISOString();
    const record = await store.save({
      familyId,
      memberId: options.actorMemberId || existing.createdByMemberId || "me",
      record: {
        ...existing,
    ...
    const executionReceipt = {
      actionId: action.id,
      eventId: options.rawEventId || `resource-owner-${record.id}-${Date.now()}`,
      executedAt,
      recordId: record.id
    };
    return {
      ...
      status: "updated",
      result: {
        ...automationDisplay("resource_list", "resource_item", { dismissible: true }),
        executionReceipt,
        record,
        text: `已把《${record.title}》的归属改为${owner.displayName}。`
      }
    };
  }
  ...
  if (action.id === "app.chat") {
    ...
    await appendConversationTurn({
      ...
      dataDir,
      entityState: requestContext.entityState,
      familyId: options.familyId || null,
      now,
...
function isDeepSummaryActionId(actionId: string): actionId is
  ...
  return (
    ...
    actionId === "summary.family.yearly"
  );
}
...
async function runDeepSummaryAutomationAction(actionId: AutomationActionId, options: AutomationRunnerOptions): Promise<AutomationRunResult> {
  ...
  if (isLiteBackend()) {
    saveLiteFamilyRecord(input.familyId, input.actorMemberId || options.actorMemberId || "me", {
      createdByMemberId: input.actorMemberId || options.actorMemberId || "me",
      id: `summary-${result.summaryId}`,
      kind: "note",
      occurredAt: input.endTime,
      ownerName: DEFAULT_ASSISTANT_NAME,
      status: "saved",
      summary: text,
      tags: ["AI总结", summaryTypeLabel(input.summaryType), ...(input.actorName ? [input.actorName] : [])],
...
function buildDeepSummaryInput(actionId: string, options: AutomationRunnerOptions): GenerateDeepSummaryInput {
  ...
  const summaryType = actionId.includes("yearly") ? "yearly" : actionId.includes("monthly") ? "monthly" : actionId.includes("weekly") ? "weekly" : "daily";
  ...
  return {
    ...
    actorName: readString(parameters.actor_name) || readString(parameters.actorName) || options.actorName || null,
    dataDir: options.dataDir || defaultDataDir,
    endTime: readString(parameters.end_time) || readString(parameters.endTime) || range.endTime,
    familyId: readString(parameters.family_id) || readString(parameters.familyId) || process.env.FAMILY_APP_LOCAL_AUTH_FAMILY_ID || "local-family",
    scope,
...
function resolveSummaryRange(now: Date, summaryType: "daily" | "weekly" | "monthly" | "yearly") {
...
function summaryTypeLabel(summaryType: GenerateDeepSummaryInput["summaryType"]) {
  if (summaryType === "yearly") return "年度总结";
  if (summaryType === "monthly") return "月度总结";
  if (summaryType === "weekly") return "周总结";
  if (summaryType === "daily") return "日总结";
  return "自定义总结";
}
...
async function createMemorySaveCandidateAction(actionId: AutomationActionId, options: AutomationRunnerOptions): Promise<AutomationRunResult> {
  ...
  if (options.confirmed) {
    ...
    const existingMemory = await findExistingConfirmedMemory(dataDir, {
      content,
      familyId: options.familyId || null,
      sourceRawEventId
    });
    if (existingMemory) {
      const result = {
        ...automationDisplay("inline_assistant", "chat_reply", { dismissible: true, requiresConfirmation: false }),
        candidate,
        memory: existingMemory,
        record: {
          id: `memory-${readString(existingMemory.id) || "existing"}`,
          kind: "note" as const,
          title: memoryTitle(candidate.memoryType, candidate.subject, candidate.fact),
          summary: readString(existingMemory.content) || content,
          ownerName: options.actorName || "我",
          status: "saved" as const,
          updatedAt: "已保存",
          tags: ["资料", "长期记忆", memoryTypeTag(candidate.memoryType)]
        },
        text: `这条已经记住了：${readString(existingMemory.content) || content}`
      };
      await appendAutomationRunEvent(actionId, options, result);
      return {
        actionId,
    ...
    const record: FamilyRecord = {
      ...
      tags: [...new Set([
        "资料",
        "长期记忆",
        memoryTypeTag(candidate.memoryType),
        ...candidate.tags.filter((tag) => tag !== "家庭")
      ].filter(Boolean))]
    };
...
function memoryTypeTag(memoryType: Awaited<ReturnType<typeof extractKnowledgeCandidate>>["memoryType"]) {
  const labels = {
    family_fact: "家庭事实",
    habit: "习惯",
    health: "健康",
...
async function findExistingConfirmedMemory(
  dataDir: string,
  input: { content: string; familyId: string | null; sourceRawEventId: string | null }
) {
  const memories = await readJsonl(`${dataDir}/memories.jsonl`);
  const familyId = input.familyId || "";
  const normalizedContent = normalizeMemoryContent(input.content);
  return [...memories].reverse().find((memory) => {
    if (readString(memory.family_id) !== familyId) return false;
    const metadata = memory.metadata && typeof memory.metadata === "object" && !Array.isArray(memory.metadata)
      ? memory.metadata as Record<string, unknown>
      : {};
    const storedSourceId = readString(memory.source_event_id) || readString(metadata.source_raw_event_id);
    if (input.sourceRawEventId && storedSourceId === input.sourceRawEventId) return true;
    return normalizeMemoryContent(readString(memory.content)) === normalizedContent;
  });
}
...
function inferRuntimeErrorType(text: string): RuntimeErrorType | undefined {
  ...
  if (/(?:数据库|存储|磁盘|ExternalStore)/i.test(text)) return "storage";
...
function answerCasualChat(text: string, conversationContext?: ConversationContext, assistantPersona: AssistantPersona = defaultAssistantPersona()) {
  ...
  if (/(你|小饭大人|小范大人|饭米粒|豆包).*(性格|个性|脾气|风格|成长)/.test(normalized) || /(你|小饭大人|小范大人|饭米粒|豆包).*(记住|记得|记忆|会记|能记)/.test(normalized)) {
...
async function answerChatWithLocalContext(
  ...
  const assistantPersona = {
    ...await readAssistantPersona(dataDir, memberId),
    growth: await evolveAssistantGrowth({ context: trustedContext, dataDir, familyId, now })
  };
  ...
  const imageToolAnswer = await answerLatestImageQuestionWithLangChainTool(text, dataDir, familyId);
  if (imageToolAnswer) {
    return { contextUsage: trustedAssistantContextUsage(trustedContext), text: imageToolAnswer };
  }

  const relationshipAnswer = await answerFamilyRelationshipRecall(text, trustedContext, conversationContext, dataDir, familyId);
  if (relationshipAnswer) {
    return { contextUsage: trustedAssistantContextUsage(trustedContext), text: relationshipAnswer };
  }

  const activeResourceAnswer = await answerActiveResourceQuestionWithLangChainTool(text, dataDir, familyId, conversationContext);
  if (activeResourceAnswer) {
    return { contextUsage: trustedAssistantContextUsage(trustedContext), text: activeResourceAnswer };
  }

  const resourceAnswer = answerRecentResourceQuestion(text, trustedContext);
  if (resourceAnswer) {
    return { contextUsage: trustedAssistantContextUsage(trustedContext), text: resourceAnswer };
  }

  const familyOverview = answerRecentFamilyOverview(text, trustedContext);
  if (familyOverview) {
    return { contextUsage: trustedAssistantContextUsage(trustedContext), text: familyOverview };
  }
  ...
  if (modelAnswer.unavailable || !modelAnswer.text) {
    ...
    return {
      contextUsage,
      text: flashFallback || (hasDeepSeekConfiguration() ? buildSafeConversationFallback(text, conversationContext) : aiUnavailableReply())
    };
  }
...
async function answerLatestImageQuestionWithLangChainTool(text: string, dataDir: string, familyId: string) {
  if (!/(?:图片|照片|截图|图里|画面).{0,10}(?:有啥|有什么|内容|文字|写了|看看|看下)|(?:看看|看下|读一下).{0,8}(?:图片|照片|截图)/.test(text)) {
    return "";
  }
  const records = await createFamilyRecordStore().list(familyId).catch(() => []);
  const imageRecord = records.find((record) => !isSyntheticResourceRecord(record) && (record.sourceFiles || []).some(isImageFile));
  if (!imageRecord) return "最近没有找到带图片附件的家庭资料。";

  try {
    const { createFamilyAutomationTools } = await import("./langchainTools");
    const imageTool = createFamilyAutomationTools({ dataDir, familyId })
      .find((candidate) => candidate.name === "family_image_ocr") as
      | { invoke(input: { record_id: string }): Promise<unknown> }
      | undefined;
    if (!imageTool) return "图片读取工具暂时不可用。";
    const rawResult = await imageTool.invoke({ record_id: imageRecord.id });
    const parsed = JSON.parse(String(rawResult)) as { content?: string; usedOcr?: boolean };
    const content = readString(parsed.content).replace(/\n{3,}/g, "\n\n").slice(0, 1200);
    if (!content) {
      return `我已经读取了《${imageRecord.title}》，但 OCR 没识别出可用文字。目前这个工具只可靠提取图片里的文字，不能凭空判断画面中的人物或物体。`;
    }
    return `我用图片 OCR 读取了《${imageRecord.title}》：\n\n${content}`;
  } catch (error) {
    return `找到了《${imageRecord.title}》，但图片 OCR 这次失败了：${error instanceof Error ? error.message : "暂时无法解析"}`;
  }
}

async function answerActiveResourceQuestionWithLangChainTool(
  text: string,
  dataDir: string,
  familyId: string,
  conversationContext?: ConversationContext
) {
  if (!/(?:里面|其中|这(?:个|份|张)?|刚才(?:那个|上传的)?).{0,8}(?:说的啥|说了什么|写了什么|什么内容|讲了什么|内容|总结|看看|读一下|解析)|^(?:里面说的啥|里面有啥|说的啥)[。.!！?？]*$/.test(text.trim())) {
    return "";
  }
  const recordId = conversationContext?.entityState.activeResourceId;
  if (!recordId) return "你指的是哪一份资料？可以点一下资料，或直接说文件名。";

  try {
    const { createFamilyAutomationTools } = await import("./langchainTools");
    const parseTool = createFamilyAutomationTools({ dataDir, familyId })
      .find((candidate) => candidate.name === "family_parse_resource") as
      | { invoke(input: { record_id: string }): Promise<unknown> }
      | undefined;
    if (!parseTool) return "资料解析工具暂时不可用。";
    const rawResult = await parseTool.invoke({ record_id: recordId });
    const parsed = JSON.parse(String(rawResult)) as { content?: string; record?: { title?: string } };
    const content = formatParsedResourceContent(readString(parsed.content));
    const title = readString(parsed.record?.title) || conversationContext?.entityState.activeResourceTitle || "这份资料";
    return content
      ? `我读取了《${title}》：\n\n${content}`
      : `我读取了《${title}》，但没有提取到可用文字。可能是扫描件质量较低，或文件本身没有文本层。`;
  } catch (error) {
    return `我找到了当前资料，但这次解析失败了：${error instanceof Error ? error.message : "暂时无法解析"}`;
  }
}

function formatParsedResourceContent(value: string) {
  const lines = value
    .replace(/\r/g, "")
    .split("\n")
  ...
  const selected = uniqueLines.slice(0, 12);
  const body = selected.map((line) => `- ${line}`).join("\n");
  return body.length > 1400 ? `${body.slice(0, 1397)}…` : body;
}

function isImageFile(file: { name: string; type?: string }) {
  return /^image\//i.test(file.type || "") || /\.(?:avif|heic|heif|jpe?g|png|webp)$/i.test(file.name);
}

function isSyntheticResourceRecord(record: FamilyRecord) {
  return /(?:^fixture-|虚构测试|synthetic|测试数据)/i.test(`${record.id} ${record.title} ${(record.tags || []).join(" ")}`);
}

async function answerFamilyRelationshipRecall(
  text: string,
  ...
  const normalized = text.trim();
  const kind = /(?:我)?(?:老婆|媳妇儿?|妻子|太太|爱人).{0,8}(?:是谁|叫啥|叫什么|名字)|(?:谁|叫什么|叫啥).{0,8}(?:是)?我(?:老婆|媳妇儿?|妻子|太太|爱人)/.test(normalized)
    ? "wife"
    : /(?:我)?(?:老公|丈夫|先生|爱人).{0,8}(?:是谁|叫啥|叫什么|名字)|(?:谁|叫什么|叫啥).{0,8}(?:是)?我(?:老公|丈夫|先生|爱人)/.test(normalized)
      ? "husband"
      : null;
  if (!kind) return "";

  const aliases = kind === "wife" ? "老婆|媳妇儿?|妻子|太太|爱人" : "老公|丈夫|先生|爱人";
  const confirmedPattern = new RegExp(`(?:^|[：:,，\\s])(?<name>[\\u4e00-\\u9fff·]{2,6})是我(?:的)?(?:${aliases})(?:$|[。.!！?？,，\\s])`);
  for (const memory of context.confirmedMemories) {
    const name = normalizeRelationshipName(memory.text.match(confirmedPattern)?.groups?.name);
    if (isLikelyRelationshipName(name)) {
      return `按你确认过的家庭资料：你的${kind === "wife" ? "老婆" : "丈夫"}叫${name}。`;
    }
  }

  const recentPattern = new RegExp(`(?:^|[，,。.!！?？：:\\s])(?:收到|明白|原来|那|所以)?[，,\\s]*(?<name>[\\u4e00-\\u9fff·]{2,6})是你(?:的)?(?:${aliases})(?:$|[，,。.!！?？：:\\s])`);
  for (const turn of [...(conversationContext?.activeTurns || [])].reverse()) {
    const name = normalizeRelationshipName(turn.assistantText.match(recentPattern)?.groups?.name);
    if (isLikelyRelationshipName(name)) {
      return `按你刚才确认的：你的${kind === "wife" ? "老婆" : "丈夫"}叫${name}。`;
    }
  }
  const legacyName = await readLegacyConfirmedRelationshipName(dataDir, familyId, kind);
  if (legacyName) {
    return `按你之前确认过的对话：你的${kind === "wife" ? "老婆" : "丈夫"}叫${legacyName}。`;
  }
  return `目前没有找到你${kind === "wife" ? "老婆" : "丈夫"}姓名的已确认资料。`;
}

function isLikelyRelationshipName(value?: string) {
  return Boolean(value && value.length >= 2 && value.length <= 6 && !/^(?:这是|她是|他是|你的|我的|收到|明白|原来|所以)$/.test(value));
}
...
async function readLegacyConfirmedRelationshipName(
  dataDir: string,
  familyId: string,
  kind: "wife" | "husband"
) {
  const events = await readJsonl(`${dataDir}/raw-events.jsonl`);
  const relationAliases = kind === "wife" ? "老婆|媳妇儿?|妻子|太太|爱人" : "老公|丈夫|先生|爱人";
  const deicticPattern = new RegExp(`(?:这|她|他)(?:个)?(?:人)?(?:就)?是我(?:的)?(?:${relationAliases})`);
  const confirmed = [...events].reverse().find((event) =>
    readString(event.source_type) === "memory.confirmed" &&
    (!readString(event.family_id) || readString(event.family_id) === familyId) &&
    deicticPattern.test(readString(event.raw_text))
  );
  if (!confirmed) return "";

  const conversationId = readString(confirmed.conversation_id) || readString(
    events.find((event) => readString(event.id) === readString(confirmed.parent_event_id))?.conversation_id
  );
  if (!conversationId) return "";
  const confirmedAt = new Date(readString(confirmed.created_at)).getTime() || Number.POSITIVE_INFINITY;
  const namePattern = new RegExp(`(?:^|[，,。.!！?？：:\\s])(?:收到|明白|原来|那|所以)?[，,\\s]*(?<name>[\\u4e00-\\u9fff·]{2,6})是你(?:的)?(?:${relationAliases})(?:$|[，,。.!！?？：:\\s])`);
  const assistantEvents = events
    .filter((event) => readString(event.source_type) === "assistant_output")
    .filter((event) => readString(event.conversation_id) === conversationId)
    .filter((event) => (new Date(readString(event.created_at)).getTime() || 0) <= confirmedAt)
    .sort((left, right) => readString(right.created_at).localeCompare(readString(left.created_at)));
  for (const event of assistantEvents) {
    const name = normalizeRelationshipName(readString(event.raw_text).match(namePattern)?.groups?.name);
    if (isLikelyRelationshipName(name)) return name;
  }
  return "";
}

function answerRecentResourceQuestion(text: string, context: TrustedAssistantContext) {
  if (!/(?:最近|刚才).{0,10}(?:上传|附件|资料|文件)|(?:上传|保存).{0,8}(?:了啥|了什么|哪些)|(?:什么|哪个|哪些).{0,6}(?:小票|发票|票据|收据)|(?:小票|发票|票据|收据).{0,6}(?:什么|哪个|哪些|内容)/.test(text)) {
    return "";
  }
  const titles = [...new Set(
    context.retrievedEvidence
      .filter((item) => item.sourceType === "resource")
      .map((item) => resourceEvidenceTitle(item.text))
      .filter((title) => Boolean(title) && !/(?:^fixture-|synthetic|虚构测试|测试数据)/i.test(title))
  )].slice(0, 5);
  if (!titles.length) return "最近没有查到已上传并保存的资料。";
  return `最近上传并保存了：\n${titles.map((title) => `- ${title}`).join("\n")}`;
}

function answerRecentFamilyOverview(text: string, context: TrustedAssistantContext) {
  if (!/(?:总结|回顾).{0,6}(?:家里|家庭)|(?:今天|最近|这周|本周).{0,8}(?:家里|家庭).{0,6}(?:有事|发生|情况)|(?:家里|家庭).{0,6}(?:今天|最近).{0,6}(?:有事|发生|情况)/.test(text)) {
    return "";
  }
  const evidence = uniqueEvidence(context.retrievedEvidence)
    .filter((item) => ["summary", "task", "resource", "message"].includes(item.sourceType))
    .filter((item) => !isInternalFamilyAutomationText(item.text))
    .slice(0, 6);
  if (!evidence.length) return "目前没有查到这段时间新增的家庭事项。";
  const summary = evidence.find((item) => item.sourceType === "summary");
  const items = evidence.filter((item) => item !== summary).slice(0, 5);
  return [
    summary ? `家庭整理：${compactEvidenceText(summary.text)}` : "",
    items.length ? `最近事项：\n${items.map((item) => `- ${compactEvidenceText(item.text)}`).join("\n")}` : ""
  ].filter(Boolean).join("\n\n");
}

function uniqueEvidence(items: TrustedAssistantContext["retrievedEvidence"]) {
  const seen = new Set<string>();
  return items.filter((item) => {
    const key = `${item.sourceType}:${item.text.replace(/\s+/g, " ").trim()}`;
...
function compactEvidenceText(text: string) {
  return text.replace(/^上传附件[：:]\s*/, "").replace(/\s+/g, " ").trim().slice(0, 160);
}

function resourceEvidenceTitle(text: string) {
...
function isInternalFamilyAutomationText(text: string) {
  const normalized = text.trim();
  return (
    /^(?:今天|最近|这周|本周).*(?:家里|家庭).*(?:有事|发生|情况)/.test(normalized) ||
    /请帮我核对|想直接向你确认|家庭助手只把本人回复|未经确认不会写入|没有明确对象，先保留/.test(normalized) ||
    /主要围绕[“"]你好啊[”"]展开/.test(normalized) ||
    /^(?:右滑完成|你在么|你好啊)$/.test(normalized)
  );
}

function aiUnavailableReply() {
  return hasDeepSeekConfiguration()
    ? "AI 回复暂时不可用，请再发一次。"
    : "请先在设置中接入 AI 服务，开启更智慧的家庭生活。";
}

export function buildSafeConversationFallback(text: string, conversationContext?: ConversationContext) {
  const normalized = text.replace(/\s+/g, "").replace(/[。.!！?？]+$/, "");
  const memberOnly = normalized.match(/^(妈妈|妈|老妈|爸爸|爸|老爸|老婆|媳妇|老公|姐姐|妹妹|哥哥|弟弟|儿子|女儿|闺女)$/)?.[1];
  const previousUserText = [...(conversationContext?.activeTurns || [])]
    .reverse()
    .find((turn) => turn.userText.trim() && turn.userText.trim() !== text.trim())
    ?.userText || "";

  if (memberOnly && /(派任务|任务|分工|交给|安排)/.test(previousUserText)) {
    return `明白，你指的是${memberOnly}。把具体任务和时间说完整，我可以整理成待确认的任务候选，不会直接创建。`;
  }
  if (/(?:谁|哪位).{0,8}(?:合适|适合)|(?:合适|适合).{0,8}(?:谁|哪位)/.test(normalized)) {
    return "我还不了解家里谁更愿意做这件事，不能随便替大家下结论。你可以告诉我几个候选人，我帮你整理比较。";
  }
  if (conversationContext?.activeTurns.length || conversationContext?.summaryText) {
    return "我刚才没能可靠理解这句话。你可以补充一下指的是谁或哪件事，我会结合刚才的对话继续。";
  }
  return "我刚才没能可靠理解这句话。你可以换一种说法，我会继续帮你梳理。";
}
...
async function answerConversationFallbackWithFlash(text: string, dataDir: string) {
  ...
  const verification = verifyConversationOnlyReplyLocally(
    text,
    normalized,
    { evidenceIds: [], executionClaims: [], grounding: "user_text" }
  );
  if (!verification.ok) {
    await recordConversationReplyRejection(dataDir, "flash_fallback", verification.reason || "policy_rejected");
    return "";
  }
  return normalized;
}
...
function formatAssistantIdentityAnswer(persona: AssistantPersona) {
  ...
  const growthText = persona.growth ? `我现在处于“${persona.growth.stageLabel}”阶段。` : "";
  return `我是${persona.displayName}，也是这个家里的 AI 家庭助手和家庭成员。${renameText}你为我设置的基础个性是：${persona.personality}。${growthText}我不会假装记得没有证据的事。`;
}

function formatAssistantPersonaAnswer(persona: AssistantPersona) {
  const growthText = persona.growth
    ? `目前的成长阶段是“${persona.growth.stageLabel}”，逐渐形成了：${persona.growth.learnedTraits.join("、")}。`
    : "";
  return `${persona.displayName}对你采用的基础个性是：${persona.personality}。${growthText}基础个性由你控制，成长只根据可信家庭经历缓慢影响表达方式；名字仍是全家共用的同一个名字。`;
}
...
export function shouldUseDeterministicCasualReply(text: string) {
  const normalized = text.trim();
  const asksAssistantPersona = /(你|小饭大人|小范大人|饭米粒|豆包).*(性格|个性|脾气|风格|成长)/.test(normalized);
      ...
      [
        {
          role: "system",
          content:
            `你是家庭 App 里的 AI 家庭成员，当前全家共用名字是「${assistantPersona.displayName}」。你原始成员 id 是 fanmili，名字可以被用户改掉，但身份连续。当前正在对话的成员是「${actor.name || actor.id || "当前成员"}」，不要用 AI 自己的名字称呼用户。当前成员为你单独设置的基础个性是「${assistantPersona.personality}」，在不违反安全与事实约束的前提下遵循它。${assistantGrowthPrompt(assistantPersona.growth)}
...
健康对话必须明确区分“用户刚说的事实”“仍需确认的信息”“一般性建议”；不得从症状直接猜疾病名称，不得把一次测量当诊断，也不得擅自声称已经提醒或建任务。`
    ...
    if (!result || typeof result !== "object" || Array.isArray(result)) {
      await recordConversationReplyRejection(dataDir, "reply_contract", result ? "invalid_contract" : "empty_response");
    ...
    if (answer.length < 4 || !executionClaims || executionClaims.length > 0) {
      await recordConversationReplyRejection(dataDir, "reply_contract", "invalid_contract");
    ...
    if (!verification.ok) {
      await recordConversationReplyRejection(dataDir, "reply_contract", verification.reason || "policy_rejected");
      return { text: null, unavailable: false };
    }
    return { text: answer, unavailable: false };
  } catch (error) {
    await recordConversationReplyRejection(dataDir, "reply_contract", "exception", error);
    return { text: null, unavailable: true };
  }
}

async function recordConversationReplyRejection(dataDir: string, stage: string, reason: string, error?: unknown) {
  await recordRuntimeEvent({
    dataDir,
    error: error || new Error(`invalid response: ${reason}`),
    event: "response.rejected",
    metadata: {
      operation: `assistant.chat.${stage}`,
      reason,
      stage
    },
    source: "ai.assistant",
...
async function readAppAnswerMembers(dataDir: string, familyId?: string | null): Promise<FamilyMember[]> {
