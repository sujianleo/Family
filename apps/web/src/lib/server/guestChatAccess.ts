...
export async function resolveGuestChatRoom(slug: string): Promise<GuestChatRoom | null> {
  ...
  return {
    ...
    id: record.id,
    familyId: ("familyId" in record && typeof record.familyId === "string" ? record.familyId : "") || "local-family",
...
async function readStoredGuestRecord(slug: string): Promise<(FamilyRecord & { familyId?: string }) | null> {
  ...
  if (externalStore) {
    const { data } = await externalStore.from("family_records").select("id, family_id, space_id, title, metadata").eq("audience", "guest").order("updated_at", { ascending: false }).limit(500);
  ...
  try {
    ...
    // The production path uses ExternalStore; a missing local fallback is expected.
  }
  return null;
}
...
export async function restoreAuthenticatedGuestChat(request: Request, slug: string) {
  ...
  const service = createServiceExternalStoreClient();
