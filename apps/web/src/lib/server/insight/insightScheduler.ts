...
async function discoverFamilyIds(): Promise<string[]> {
  ...
  if (!externalStore) return ["local-family"];
  const { data, error } = await externalStore.from("families").select("id").limit(500);
  if (error) throw error;
  return [...new Set<string>((data || []).map((row: any) => String(row.id || "")).filter(Boolean))];
}
