...
async function runAutomationActionInternal(actionId: string, options: AutomationRunnerOptions = {}): Promise<AutomationRunResult> {
  ...
  if (action.id === "resource.assign_owner") {
    const familyId = options.familyId || process.env.SUPABASE_DEFAULT_FAMILY_ID || "local-family";
...
function buildDeepSummaryInput(actionId: string, options: AutomationRunnerOptions): GenerateDeepSummaryInput {
  ...
  return {
    ...
    familyId: readString(parameters.family_id) || readString(parameters.familyId) || process.env.FAMILY_APP_LOCAL_AUTH_FAMILY_ID || process.env.SUPABASE_DEFAULT_FAMILY_ID || "local-family",
    scope,
