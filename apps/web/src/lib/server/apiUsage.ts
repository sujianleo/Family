...
export async function recordApiUsage(input: ApiUsageInput) {
  ...
  await insertExternalStoreApiUsage(row);
...
async function insertExternalStoreApiUsage(row: Record<string, unknown>) {
  const externalStore = createServiceExternalStoreClient();
  if (!externalStore || !row.family_id) {
    return;
  }
  const { error } = await externalStore.from("api_usage").insert(stripUndefined(row));
  if (error) {
