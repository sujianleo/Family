...
export async function createRawEvent(input: RawEventInput) {
  const event = {
    ...
    family_id: normalizeUuid(input.familyId || process.env.SUPABASE_DEFAULT_FAMILY_ID),
  ...
  await insertSupabaseRow("raw_events", event);
...
export async function createAssistantInterpretation(input: AssistantInterpretationInput) {
  ...
  await insertSupabaseRow("assistant_interpretations", interpretation);
...
export async function createAutomationRun(input: AutomationRunInput) {
  ...
  await insertSupabaseRow("automation_runs", run);
...
export async function createSummary(input: SummaryInput) {
  ...
  await insertSupabaseRow("summaries", summary);
...
async function insertSupabaseRow(table: string, row: Record<string, unknown>) {
  ...
  const { error } = await supabase.from(table).insert(stripUndefined(row));
  if (error) {
